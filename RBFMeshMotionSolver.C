...
(
    ...
    :
    displacementMotionSolver(mesh, msData, typeName),
...
{
    pointDisplacement_.primitiveFieldRef() = newPoints;
...
{
    // first let's move the boundaries
    pointDisplacement_.correctBoundaryConditions();

    Field<vectorField> movingPatchesDisplacement(mesh().boundaryMesh().size(), vectorField( 0 ));

    forAll( movingPatchIDs, patchId )
    {
        vectorField displacementPoints = pointDisplacement_.boundaryField()[movingPatchIDs[patchId]].patchInternalField();
        if (faceCellCenters)
        {
            PrimitivePatchInterpolation<polyPatch> pointToFaceInterpolator (mesh().boundaryMesh()[movingPatchIDs[patchId]]);
            movingPatchesDisplacement[movingPatchIDs[patchId]] = pointToFaceInterpolator.pointToFaceInterpolate(displacementPoints);
        }
        else
        {
            movingPatchesDisplacement[movingPatchIDs[patchId]] = displacementPoints;
        }
    }
    setMotion( movingPatchesDisplacement );
...
//        Something wrong, index == nbGlobalStaticFaceCenters[Pstream::myProcNo() - 1
//        when nbGlobalStaticFaceCenters[Pstream::myProcNo()] > 0
//
...
//        Something wrong, index == nbGlobalFixedFaceCenters[Pstream::myProcNo() - 1
//        when nbGlobalFixedFaceCenters[Pstream::myProcNo()] > 0
//
